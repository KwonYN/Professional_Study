...
/*
	[Re]
	M개의 라인에 각각 N개의 부품을 순차적으로(N을 시간이라고 봐도 무방)
	K명의 엔지니어가 투입(다 투입시킬 필요 X, 같은 부품 담당 여러 명일 수도)
*/
#if 0
...
void input_() {
	...
	for (int i = 1; i <= M; i++) {
		...
		for (int j = 1; j <= N; j++) {
			...
			/*
				sum_alpha[ ][j] : j번째 시간(열)에
				sum_alpha[char_to_int(input_tmp[j])][ ] : 주어진 부품의 개수 세기
			*/
		}
	}
